...
// 
// GFX_THROW_INFO_ONLY(context_->DrawIndexed((UINT)std::size(indices), 0u, 0u));
// 
// infoManager.Set(); (context_->DrawIndexed((UINT)std::size(indices), 0u, 0u)); 
// {
// 	auto v = infoManager.GetMessages(); if (!v.empty())
// 	{
// 		throw GraphicsModule::InfoException(__LINE, __FILE__,v);}
// 	};
// }